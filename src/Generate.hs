...
menu (Queries qs) =
  ...
  where
    lis :: [Text]
    lis =
        for qs $ \(SomeQuery qname _q) ->
            T.concat $
                [ "<h5 class=\"category\">" <> T.pack qname <> "</h5>"
                , ul $ for universe $ \qparam ->
                    let _ = _q undefined [qparam] -- NOTE: Only used for type inferance of qparam.
                     in "<button class=\""
                            <> T.pack qname
                            <> "\" id=\""
                            <> T.concat (display qparam)
                            <> "\">"
                            <> T.replace "_" " " (T.concat (display qparam))
                            <> "</button>"
                ]
...
displayRecipe (Recipe name meals mKitchen diets ingredients_ mInstructions mSource mUrl) =
    T.unlines $
        [ h3 (anchor name <> (if Vegan `elem` diets then " <font color=green>Ⓥ</font>" else ""))
        , italic $
            parenthesis $
                maybe
                    mealsText
...
anchor :: Text -> Text
anchor t = "<a id=\"" <> ident <> "\" href=\"#" <> ident <> "\">" <> t <> "</a>"
  where
    ident = T.replace " " "-" t
